...
func (urq *UserRoleQuery) QueryTenant() *TenantQuery {
	query := (&TenantClient{config: urq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := urq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (urq *UserRoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*UserRole, error) {
	...
	if query := urq.withTenant; query != nil {
		if err := urq.loadTenant(ctx, query, nodes, nil,
			func(n *UserRole, e *Tenant) { n.Edges.Tenant = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (urq *UserRoleQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*UserRole, init func(*UserRole), assign func(*UserRole, *Tenant)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*UserRole)
	for i := range nodes {
...
func (urq *UserRoleQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := urq.ctx.Fields; len(fields) > 0 {
		...
		if urq.withTenant != nil {
			_spec.Node.AddColumnOnce(userrole.FieldTenantID)
		}
	}
